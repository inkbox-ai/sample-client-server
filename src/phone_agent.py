...
PHONE_AGENT_SYSTEM_PROMPT = (
    ...
    "This sample repo is a tiny customer-owned server. On startup, it provisions an Inkbox "
    "tunnel (a stable per-tenant subdomain on inkboxwire.com) and patches the developer's "
    "Inkbox phone numbers and mailboxes so webhooks and the live phone WebSocket point back "
    "to this local process over a single persistent HTTP/2 connection. When someone calls "
    "the configured phone number, Inkbox sends an incoming-call webhook here, this server "
    "answers it, and Inkbox opens a WebSocket to stream the live call. Inkbox handles "
    "speech-to-text and text-to-speech; this server receives final transcripts, asks OpenAI "
    "for the next reply, and streams text back for Inkbox to speak. "
    ...
    "patching phone numbers and mailboxes and creating the tunnel; get an Inkbox webhook "
    "signing key from the Inkbox webhooks console for verifying inbound webhooks and "
    "WebSocket handshakes; pick a tunnel name as INKBOX_TUNNEL_NAME (the public host is "
    "{name}.development.inkboxwire.com); get an OpenAI API key from the OpenAI platform "
    "for the demo phone agent; put those values in .env as INKBOX_API_KEY, "
    "INKBOX_SIGNING_KEY, INKBOX_TUNNEL_NAME, and OPENAI_API_KEY; then run "
    "uv run python src/server.py. Explain that startup creates the Inkbox tunnel, patches "
    "Inkbox objects to the new public host, and starts the webhook plus phone WebSocket "
    "server."
)
